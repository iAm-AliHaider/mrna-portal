...
import { useEmployees, useEmploymentType } from "../../../../utils/hooks/api/endContractRequests";
...
const AutoFillLastWorkingDate = ({ employees, employmentTypeById }) => {
  ...
  React.useEffect(() => {
    ...
    const effectedDate = parseISO(String(effected));
  ...
  }, [values?.effected_date, values?.employee_id, employees, employmentTypeById, setFieldValue]);

  return null;
};

const NewEndContractRequestForm = ({
  open,
  onClose,
  id = null,
  handleSubmit,
  ...
  const filteredEmployees = useMemo(() => {
    if (!Array.isArray(employees) || !Array.isArray(employmentTypeData) || employmentTypeData.length === 0) {
      return [];
    }
  ...
  const computeLastWorkingDay = (selectedEmployee) => {
    ...
    const noticeDays = Number.isFinite(Number(noticeDaysRaw)) ? Number(noticeDaysRaw) : 0;
  ...
  return (
    <Modal
      onClose={onClose}
      title={id ? "Edit Contract End Request" : "New Contract End Request"}
      open={open}
    >
      <div className="flex flex-col">
        <Formik
          ...
          validationSchema={resignationValidationSchema(userRole, "contract")}
                ...
                onChange={(selectedEmployee) => {
                  ...
                  if(selectedEmployee){
                    ...
                    filteredEmployees.find((e) => e.id === selectedEmployee);
                      ...
                      const lwd = computeLastWorkingDay(fullEmployee);
                      setFieldValue("last_working_date", lwd);
                  }
                  else{
...
export default NewEndContractRequestForm;

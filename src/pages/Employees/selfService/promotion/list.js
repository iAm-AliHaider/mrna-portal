...
import {
  ...
  useCreatePromotion,
  useUpdatePromotion,
} from "../../../../utils/hooks/api/usePromotions";
...
const PromotionPage = () => {
  ...
  const { data, totalPages, loading, refetch } = usePromotions(
    page,
    searchQuery,
    filters,
    perPage
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      if (selectedId) {
        ...
        await transactionEmailSender(user, payload, "Promotion", "Promotion Request");
      }
      ...
      >
        <div className="bg-white p-4 rounded-lg shadow-md flex flex-col gap-4">
          ...
          <DynamicTable
            columns={columns}
            data={data}
            footerInfo={`Promotion Requests: ${data.length}`}
            currentPage={page + 1}

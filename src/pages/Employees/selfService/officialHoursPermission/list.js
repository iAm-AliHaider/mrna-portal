...
import OfficialHoursPermissionForm from "./form";
import {
  useOfficialHoursPermission,
  useCreateOfficialHoursPermission,
  useUpdateOfficialHoursPermission,
} from "../../../../utils/hooks/api/useOfficialHoursPermission";
...
const breadcrumbItems = [
  ...
  { title: "Self Service" },
  { title: "Official Hours Permission" },
];

const OfficialHoursPermissionPage = () => {
  ...
  const { data, totalPages, loading, refetch } = useOfficialHoursPermission(
    page,
    searchQuery,
    filters,
    perPage
  ...
  const { create } = useCreateOfficialHoursPermission();
  const { update } = useUpdateOfficialHoursPermission();
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      if (selectedId) {
        ...
        await transactionEmailSender(user, payload, "Official Hours Permission", "Official Hours Permission Request");
      }
      ...
      >
        <div className="bg-white p-4 rounded-lg shadow-md flex flex-col gap-4">
          ...
          <DynamicTable
            columns={columns}
            data={data}
            footerInfo={`Official Hours Permissions: ${data.length}`}
            currentPage={page + 1}
...
export default OfficialHoursPermissionPage;

...
import AttendancePDF from "../../../../components/pdfComponenets/AttendancePDF";
import DownloadPDFButton from "../../../../components/pdfComponenets/DownloadPDFButton";
import useAttendanceData from "../../../../utils/hooks/api/useAttendanceData";
...
const reportNameOptions = [
  {
    value: "Default Employee Attendace Report",
    label: "Default Employee Attendace Report",
  },
];
...
const DailyAttandanceReport = () => {
  const [employeeAttendanceInfo, setEmployeeAttendanceInfo] = useState({
    month: null,
    year: null,
    employee_id: null
  });
...
setEmployeeOptions(options);
  ...
  const { data: attendanceData, error, loading: attendanceLoading } = useAttendanceData(
    employeeAttendanceInfo.month &&
      employeeAttendanceInfo.year &&
      employeeAttendanceInfo.employeeId
      ? employeeAttendanceInfo
      : null
  );

  useEffect(() => {
    if (attendanceData && !attendanceLoading && employeeDetails) {
      const formattedEmployeeData = formatEmployeeData(employeeDetails, {
        month: employeeAttendanceInfo.month,
        year: employeeAttendanceInfo.year,
        employeeNumber: employeeAttendanceInfo.employeeId,
        permanentEmailAddress: employeeAttendanceInfo.permanentEmail,
        notes: employeeAttendanceInfo.notes,
      });
  ...
  }, [attendanceData, attendanceLoading, employeeDetails, employeeAttendanceInfo]);
    ...
    <Box className="page-wrapper">
      ...
      <div className="bg-white rounded-lg shadow-md">
        ...
        {isFormSubmitted ? (
          ...
          ) : pdfData?.attendance && pdfData?.employee ? (

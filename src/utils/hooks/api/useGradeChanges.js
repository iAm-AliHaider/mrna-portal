...
export function useGradeChanges(page = 0, searchQuery = "", filters = {}, perPage = 10) {
...
export function useCreateGradeChange() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const create = useCallback(async (payload) => {
    ...
    try {
      ...
      toast.success("Grade change request submitted!");
      return data;
    } catch (err) {
...
export function useUpdateGradeChange() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const update = useCallback(async (id, payload) => {
    ...
    try {
      const { data, error: sbError } = await supabase
        ...
        .single();
      if (sbError) throw sbError;
      toast.success("Grade change request updated!");
      return data;
    } catch (err) {
...
export function useAllGradeChanges(page = 0, searchQuery = "", filters = {}, perPage = 10) {
  const [data, setData] = useState([]);
  const [totalPages, setTotalPages] = useState(0);
  const [error, setError] = useState(null);
  const [count, setCount] = useState(0);
    ...
    async (isCancelled) => {
      try {
        ...
        let query = supabase
          ...
          .select(`
            *,
            employee:employees!employee_grade_changes_employee_id_fkey(
              id,
              employee_code,

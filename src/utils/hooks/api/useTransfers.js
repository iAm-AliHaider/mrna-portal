...
export function useTransfers(page = 0, searchQuery = "", filters = {}, perPage = 10) {
...
export function useAllTransfers(page = 0, searchQuery = "", filters = {}, perPage = 10) {
  const [data, setData] = useState([]);
  const [totalPages, setTotalPages] = useState(0);
  const [error, setError] = useState(null);
  const [count, setCount] = useState(0);
    ...
    async (isCancelled) => {
      try {
        ...
        let query = supabase
          ...
          .select(`
            *,
            employee:employees!employee_transfers_employee_id_fkey(
              id,
              employee_code,

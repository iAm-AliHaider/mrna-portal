...
export function usePersonalHoursPermission(page = 0, searchQuery = "", filters = {}, perPage = 10) {
  ...
  const fetchData = useCallback(
    async (isCancelled) => {
      ...
      try {
        ...
        let query = supabase
          .from("personal_hours_permission")
          .select("*", { count: "exact" })
          .eq("is_deleted", false)
        ...
        if (queryError) {
          ...
          setTotalPages(0);
          setCount(0);
          toast.error("Failed to fetch personal hours permissions");
          return;
        }
...
export function useCreatePersonalHoursPermission() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const create = useCallback(async (payload) => {
    ...
    try {
      ...
      toast.success("Personal hours permission request submitted!");
      return data;
    } catch (err) {
...
export function useUpdatePersonalHoursPermission() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const update = useCallback(async (id, payload) => {
    ...
    try {
      const { data, error: sbError } = await supabase
        ...
        .single();
      if (sbError) throw sbError;
      toast.success("Personal hours permission updated!");
      return data;
    } catch (err) {
...
export function useDeletePersonalHoursPermission() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const deleteRecord = useCallback(async (id) => {
    ...
    try {
      const { error: sbError } = await supabase
        ...
        .eq("id", id);
      if (sbError) throw sbError;
      toast.success("Personal hours permission deleted.");
      return true;
    } catch (err) {
      setError(err.message || err);

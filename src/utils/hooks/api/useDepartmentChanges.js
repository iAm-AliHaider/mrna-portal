...
export function useDepartmentChanges(page = 0, searchQuery = "", filters = {}, perPage = 10) {
  ...
  const fetchData = useCallback(
    async (isCancelled) => {
      ...
      try {
        ...
        let query = supabase
          .from("employee_department_changes")
          .select("*", { count: "exact" })
          .eq("is_deleted", false)
...
export function useCreateDepartmentChange() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
...
export function useUpdateDepartmentChange() {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const update = useCallback(async (id, payload) => {
    ...
    try {
      const { data, error: sbError } = await supabase
        ...
        .single();
      if (sbError) throw sbError;
      toast.success("Department change request updated!");
      return data;
    } catch (err) {
...
export function useAllDepartmentChanges(page = 0, searchQuery = "", filters = {}, perPage = 10) {
  const [data, setData] = useState([]);
  const [totalPages, setTotalPages] = useState(0);
  const [error, setError] = useState(null);
  const [count, setCount] = useState(0);
    ...
    async (isCancelled) => {
      try {
        ...
        let query = supabase
          ...
          .select(`
            *,
            employee:employees!employee_department_changes_employee_id_fkey(
              id,
              employee_code,

...
export function useOffboardingTasks({
    ...
    const fetchTasks = useCallback(async () => {
        ...
        let query = supabase
            .from('assigned_tasks')
            .select(`*,
        employee:assigned_tasks_employee_id_fkey(candidate:employees_candidate_id_fkey(*)),
        ...
        assignedTo:assigned_tasks_assigned_to_id_fkey(candidate:employees_candidate_id_fkey(*))`,
            ...
            .in('tasks.task_type', ['pre_off_boarding', 'post_off_boarding'])
        ...
        if (searchTerm) {
            query = query
                .ilike('employee.candidate.full_name', `%${searchTerm}%`) // Match search term in full_name
                .not('employee.candidate', 'is', null) // Ensure candidate data exists
                .neq('employee.candidate.full_name', '')
                .not('employee', 'is', null); // Exclude empty full_name values
        }
        ...
        if (fetchError) {
            ...
            let taskData = rows.map(row => ({
                ...
                candidate_name: row?.employee?.candidate?.full_name,

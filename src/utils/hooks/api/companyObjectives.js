...
export function useAddOrganizationalObjective () {
  const { user } = useUser()
  ...
  const addOrganizationalObjective = useCallback(
    async objectiveData => {
      setLoading(true)
      setError(null)

      try {
        const { data: sumData, error } = await supabase.rpc('get_total_organizational_objective_weight')
...
export function useOrganizationalObjectivesList (
  page = 0,
  pageSize = 4,
  searchQuery = ''
) {
...
export function useDeleteOrganizationalObjective () {
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState(null)
  ...
  const deleteObjectives = useCallback(async ids => {
    ...
    try {
      let query = supabase.from('organizational_objectives')
...
export function useOrganizationalObjectiveById (objectiveId) {
...
export function useUpdateOrganizationalObjective () {
  const { user } = useUser()
  const employeeId = user?.id
  ...
  const updateOrganizationalObjective = useCallback(

...
import StatusChangeForm from "../../components/common/StatusChangeForm";
...
const StatusChangeList = ({
  ...
  const { data, totalPages, loading, refetch } = hookData(
    page,
    searchQuery,
    filters,
    perPage
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    try {
      if (selectedId) {
        ...
        await transactionEmailSender(user, payload, singularTitle, `${singularTitle} Request`);
      }
  ...
  const columns = [
    ...(formConfig.oldLabel ? [{ key: formConfig.oldValueKey, label: formConfig.oldLabel }] : []),
    ...(formConfig.newLabel ? [{ key: formConfig.newValueKey, label: formConfig.newLabel }] : []),
  ...
  return (
    <React.Fragment>
      <PageWrapperWithHeading
        ...
        action={handleCreate}
        buttonTitle={`Request ${singularTitle}`}
        Icon={AddIcon}
      >
